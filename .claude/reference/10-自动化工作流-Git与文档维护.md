...
### 2.8 Git 提交安全防护（两层 + zip 备份）

> 适用场景：多工具混用（Claude Code / Codex CLI / Trae / 用户手动改文件）时，防止盲目提交覆盖他人改动。

#### 2.8.1 为什么需要

当多个 AI 工具或用户在同一仓库交替修改时，Claude Code 的自动 checkpoint 可能：

- 把别人的改动一股脑 `git add -A` 提交
- 覆盖其他工具的未完成工作
- 在有冲突标记时仍然提交

**用 Hook 做门卫比 Skill 更合适**：Hook 是强制触发的，Skill 是被动调用的。

#### 2.8.2 两层防护架构

| 层 | Hook 类型 | 脚本 | 职责 |
|----|----------|------|------|
| 第一层 | `PreToolUse` (Bash) | `git_safety_check.py` | 拦截 `git commit/push`，分析风险，HIGH 则阻断 |
| 第二层 | `Stop` | `auto_checkpoint_commit.py` | 回复结束后分析工作区，脏则 zip 备份+跳过 |

**协作关系**：

| 场景 | 第一层(拦截) | 第二层(备份) |
|------|-------------|-------------|
| Claude 主动 git commit | ✅ 拦截评分 | - |
| CC 回复结束 | - | ✅ 检测+备份+提交 |
| 脏工作区 CC 想提交 | ✅ HIGH 阻断 | ✅ zip 兜底 |
| 用户手动 git commit | ❌ 不经过 CC | ❌ 无触发 |

#### 2.8.3 第一层：git_safety_check.py（PreToolUse）

**触发时机**：Claude Code 执行 Bash 命令**之前**

**原理**：Claude Code 通过 stdin 传入 `{"tool_input":{"command":"git commit ..."}}`，脚本解析命令，检测是否包含 `git commit` / `git push`，然后分析工作区。
...
|--------|------|------|
| 冲突标记 `<<<<<<<` | +100 | 直接 HIGH |
| 暂存+未暂存并存 | +30 | 可能只提交了部分 |
| mtime 跨度 >2min | +20 | 疑似多工具交替修改 |
| 本地落后远端 | +25 | 推送前需先 pull |
| stash 积压 >=2 | +15 | 工作区切换频繁 |
| 未跟踪文件 >5 | +10 | 需确认是否纳入 |

**评分 → 行为**：

- `>= 50` → **HIGH** → exit 2（命令被阻断，Claude 看到警告后告知用户）
- `>= 20` → **MEDIUM** → exit 0（放行，但 stderr 警告注入 Claude 上下文）
- `< 20` → **LOW** → exit 0（静默放行）

**注册方式**（settings.local.json）：
      ...
      "hooks": [
        {
          "type": "command",
          "timeout": 30,
          "command": "python -c \"import os, runpy, sys; from pathlib import Path; project_dir = Path(os.environ.get('CLAUDE_PROJECT_DIR') or os.getcwd()).resolve(); runner_path = (project_dir / '.claude' / 'scripts' / 'hook_runner.py').resolve(); sys.argv = [str(runner_path), '--script', '.claude/scripts/git_safety_check.py']; runpy.run_path(str(runner_path), run_name='__main__')\""
        }
      ]
    }
  ]
...
#### 2.8.4 第二层：auto_checkpoint_commit.py（Stop hook，已增强）

**触发时机**：Claude Code 每次回复**结束后**

**增强内容**（相比原始版本）：

1. **工作区分析**：变更文件数、多作者历史、混合变更检测、陈旧变更、冲突标记
2. **zip 备份**：脏工作区时自动打包变更文件
3. **备份分支**：自动创建 `backup/auto-before-checkpoint-*` 分支

**行为**：

```
回复结束 → 分析工作区
  干净 → git add -A && git commit（和之前一样）
  脏乱 → 三步保护：
    1. zip 打包 → .claude/backups/checkpoint-backup-YYYYMMDD-HHMMSS.zip
    2. 创建备份分支 → backup/auto-before-checkpoint-YYYYMMDD-HHMMSS
    3. 跳过自动提交 → 输出警告
```
...
```
checkpoint-backup-20260401-161500.zip
├── _manifest.txt    (备份时间、分支、变更文件清单)
├── claude.md
├── .claude/workflows/claude-workflow-constants.md
└── ... (所有变更文件原样打包)
```

**恢复方式**：解压 zip 到项目根目录即可覆盖还原。

**阈值配置**（在脚本顶部可调）：

```python
MAX_CHANGED_FILES_AUTO = 10       # 超过视为大量变更
MAX_STREAK_AGE_MINUTES = 120      # 超过视为陈旧变更
...
```bash
python .claude/scripts/auto_checkpoint_commit.py             # 正常运行
python .claude/scripts/auto_checkpoint_commit.py --dry-run   # 只分析不提交
python .claude/scripts/auto_checkpoint_commit.py --force     # 跳过检测强制提交
python .claude/scripts/auto_checkpoint_commit.py --no-zip    # 跳过 zip 备份
```

#### 2.8.5 Codex 对比

Codex 的 `/codex:rescue` 和 `/codex:review` 都是只读或任务执行性质的，没有这种**在提交前拦截并强制用户介入**的机制。这种门卫式的安全检查目前只有 Claude Code 的 Hook 系统能做到。
