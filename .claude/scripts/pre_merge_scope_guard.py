...
def emit_error(reason_text: str) -> int:
    """统一输出阻断原因。"""
    print(f"[pre_merge_scope_guard] {reason_text}", file=sys.stderr)
    return 2
...
def infer_task_name_from_branch(source_branch: str) -> str:
    ...
    if not task_match:
        return ""
    return task_match.group(1).replace("_", "-")
...
def load_tasks_from_table(table_path: Path) -> list[str]:
    ...
    return sorted({clean_cell(row["task"]) for row in rows if clean_cell(row["task"])})
...
def main() -> int:
    ...
    if not source_branch:
        return emit_error("无法解析 git merge 来源分支，请改用 'git merge <branch>' 形式")
    ...
    try:
        ...
        return emit_error(f"范围门禁未通过: {exc}")

    if verify_code != 0:
        brief_output = verify_output[-600:] if verify_output else "无详细输出"
        return emit_error(
            f"范围门禁未通过: source={source_branch}, task={task_name}\n{brief_output}"
        )

    print(
        (
            "[pre_merge_scope_guard] 范围门禁通过: "
            f"source={source_branch}, task={task_name}, table={table_path}, base={base_ref}"
        ),
        file=sys.stderr,
    )
    return 0
