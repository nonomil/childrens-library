---
name: codex-toolkit
description: 将项目 .claude/ 配置转换为 Codex CLI 兼容格式。支持插件市场和项目级转换，以及跨平台路径映射（Cursor/Gemini/Copilot/Windsurf 等）。触发信号："导出 Codex"/"部署到 Codex"/"codex export"/"codex deploy"。
layer: ondemand
...
不同 AI 编码工具使用不同的配置目录结构。本技能的转换逻辑可扩展到以下平台：

| 平台 | Skills 路径 | 配置文件 | 备注 |
|------|-----------|---------|------|
| Claude Code | `.claude/skills/` | `CLAUDE.md` | 本工具的源 |
| Codex CLI | `.agents/skills/` | `AGENTS.md` | 当前转换目标 |
| Cursor | `.cursor/skills/` | `.cursorrules` | 类似结构，需调整 frontmatter |
| Gemini CLI | `.gemini/skills/` | `GEMINI.md` | Google 生态 |
| GitHub Copilot | `.github/skills/` | `copilot-instructions.md` | 最简结构 |
| Windsurf | `.windsurf/skills/` | `.windsurfrules` | Codeium 生态 |
| OpenAI Codex | `codex/` | `codex.md` | OpenAI CLI |

### 扩展转换逻辑

当用户指定目标平台时：
1. 读取平台对应的路径映射
2. 调整 frontmatter 格式（不同平台支持的字段不同）
3. 转换配置文件格式
4. 输出到对应目录

### Frontmatter 兼容性矩阵

| 字段 | Claude Code | Codex CLI | Cursor |
|------|------------|-----------|--------|
| name | ✅ | ✅ | ✅ |
| description | ✅ | ✅ | ✅ |
| layer | ✅ | ❌ | ❌ |
