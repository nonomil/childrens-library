...
✓ `scan.py` — 根目录兼容入口
✓ `scripts/scan.py` — CLI 主入口
✓ `scripts/scan_commands.py` — 子命令实现
✓ `scripts/scan_extract_core.py` — extract 核心逻辑
✓ `scripts/scan_sidecar_views.py` — sidecar 视图构建
✓ `scripts/scan_rich_contract.py` — richer scan-data 合同生成
...
- [ ] richer contract 继续加深：入口点、数据流、影响矩阵推断
- [ ] 插件侧更充分消费 `module_inventory / data_flows / impact_matrix`
- [ ] 文档与排错材料继续瘦身，把历史草稿逐步从主 README 中剥离

## 当前目录整理状态

当前目录已经按“说明 / 实现 / 测试 / 模板 / 参考资料”分层：
...
├── references/      # 正式参考规范
├── ref/             # 历史资料
└── docs markdown    # SKILL / README / ANALYSIS
```

这样做的目标不是“为了好看”，而是为了避免继续把实现、流程、参考材料混在同一层。

补充约定：

- `references/` 保存当前仍有效的正式参考资料。
- `ref/` 保存历史讨论、旧稿和排错记录。
- 两边若出现同名文档，以 `references/` 版本为准。
