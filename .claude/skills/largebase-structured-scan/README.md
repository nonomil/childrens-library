...
- skill 根目录保留 `SKILL.md`、`README.md`、`schema.json`、`templates/`、`references/` 等说明和资源文件
- Python 实现统一收敛到 `scripts/`
- 根目录 `scan.py` 是兼容入口，外部命令仍可继续使用 `python .claude/skills/largebase-structured-scan/scan.py ...`
- `tests/` 保留在 skill 根目录，用于同时覆盖“兼容入口 + scripts 实现”这两层行为

## 目录结构速览

```text
largebase-structured-scan/
├── SKILL.md                 # skill 触发条件与执行流程
├── README.md                # 设计说明、目录职责、命令示例
├── ANALYSIS.md              # 架构分析与方案演进记录
├── scan.py                  # 兼容入口，转发到 scripts/scan.py
├── schema.json              # scan-data / SQLite 相关 schema 约定
├── scripts/                 # Python 实现代码
├── tests/                   # pytest 与固定夹具
├── templates/               # M1-M4 Prompt 模板
├── references/              # 当前流程依赖的参考规范
├── ref/                     # 历史讨论、旧稿、排错记录
└── agents/                  # 外部代理配置
```
...
- 改 CLI 参数、子命令注册：编辑 `scripts/scan.py`
- 改 extract / sidecar / richer contract：优先编辑 `scripts/` 下对应模块，不要把逻辑重新塞回根目录 `scan.py`
- 改 Prompt 模板：编辑 `templates/`
- 改规范或输出合同：编辑 `references/` 与 `schema.json`
- 改测试：编辑 `tests/`，默认继续通过根目录 `scan.py` 验证兼容层
- 查旧设计、旧问题和历史材料：先看 `ANALYSIS.md`，再看 `ref/`

## scripts 模块分工

更细的模块职责见 [scripts/README.md](./scripts/README.md)。

## 资料导航

- 当前正式参考：`references/`
  说明见 [references/README.md](./references/README.md)
- 历史讨论和排错记录：`ref/`
  说明见 [ref/README.md](./ref/README.md)

如果两个目录里出现同名材料，例如 `output-contract.md`，默认以 `references/` 中的版本为准。
...
为了让 IDE 插件在不破坏现有扫描结果的前提下展示“总览图 / 模块图 / 流程图”，`extract` 阶段会额外生成三份 sidecar JSON：

- `anchors.json`
- `module_views.json`
- `flow_views.json`

这三份文件都是**增量产物**，不会替换现有的 `scan-data.json` 或 `extract-data.json`。

当前阶段只保证下面三类 sidecar：

- `anchors`：模块、流程、符号、文件的代表跳转锚点
- `module_views`：默认总览图和一层子模块视图
- `flow_views`：少量核心流程步骤图

插件侧必须遵循这条回退规则：

- sidecar 存在时优先读取 sidecar
- sidecar 缺失时自动回退到旧版 `module_graph / data_flows` 视图

这意味着老的扫描目录不用重扫也能继续打开，只是不会获得新的分层图能力。
