...
| # | 门禁点 | 位置 | 通过条件 |
|---|--------|------|----------|
| G1 | 读取全部 section 标题 | 全文 | 输出所有 ## 和 ### 标题 |
| G2 | doc-gen 校验清单 | Step 2 结束前 | 图表数量达标：01≥3, 02≥3, 03≥2, 04≥2, 05≥3, 06≥3 |
| G3 | scan.py verify | Step 2 结束后 | 退出码 = 0 |
...
维护约定：
- Python 实现位于 `scripts/`
- 根目录 `scan.py` 为兼容入口，供外部命令和旧文档继续调用
- `tests/` 留在 skill 根目录，优先从兼容入口验证整体行为

## Skill Layout

- 入口与说明：`SKILL.md`、`README.md`、`scan.py`
- 代码实现：`scripts/`
- 模板与合同：`templates/`、`references/`、`schema.json`
- 测试与夹具：`tests/`
- 历史分析：`ANALYSIS.md`、`ref/`
...
### Step 0：可选增强 — Graphify / Cartographer 检测

> 此步骤为**可选增强**，不阻塞扫描流程。

1. 若用户显式说“创建知识图谱”“生成知识图谱”“graphify”“代码图谱”，先执行：
...
2. 若 `docs/代码库-知识图谱/{项目名}-LLM图谱/GRAPH_REPORT.md` 已生成，可将其作为快速预热材料读取，但不要把它当作 `docs/CODEBASE_MAP.md` 的替代品
3. 检查 `docs/CODEBASE_MAP.md` 是否存在且 `Generated at` ≤ 7 天
4. **若可用**：提示用户 `"检测到 CODEBASE_MAP.md 可用，将作为参考文档 --refs 传入"`，直接进入 Step 1
5. **若不可用**：提示用户选择：
   ```
   未检测到 CODEBASE_MAP.md，请选择：
     ...
     C) 跳过，直接进入扫描
   ```
6. 选 A → Step 1 后自动执行 Step 1.5
7. 选 B → 尝试执行 Cartographer，成功则用其产物，失败则回退到 A
8. 选 C → 直接进入 Step 1
