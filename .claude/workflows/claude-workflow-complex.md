...
Phase 0.5 （可选）Debate 机制 — 需求阶段对抗式讨论
         - 触发条件：`debate_strategy` 偏好为 `on_demand`（复杂任务）或 `always`
         - 跳转 `claude-workflow-debate.md`
         - 产出：结构化需求共识文档（4 层讨论 + 回溯校验）
         - 共识文档作为 Phase 1 Plan 生成的输入
         - 简单模式或 `debate_strategy=off` → 跳过

Phase 1  CC 生成 Plan 文档（docs/plan/YYYY-MM-DD-[feature].md）
         → 基于 Debate 共识文档（如有）或扫描结果生成 Plan
         → 向用户展示 Plan，等待用户确认内容
...
Phase 6  用户明确说"开始开发"后，进入执行阶段
         → 跳转 `claude-workflow-conductor.md`（Task 级增量执行）
         → 每个 task 用户选择执行者（CC / Codex / 跳过 / 用户自做）
         → 不默认交给 Codex
```
...
在进入任何 Phase 之前，用 Grep 搜索 `.claude/memory/` 中的历史经验：
```
Grep({ pattern: "[当前任务核心关键词]", path: ".claude/memory/" })
```
目的：复用过往方案，避免重复犯错。搜索结果作为 Phase 1 Plan 的输入之一。
...
- [ ] Phase 6：用户说"开始开发"，进入 Conductor 执行阶段
...
每个 steps.md 文件必须以 frontmatter 开头（context 按需声明原则）：

---
context:
  - .claude/rules/[最相关的规范文件]          # 按需声明，不超过5个
  - docs/[任务直接依赖的参考文档]              # 只列本任务真正会用到的
---

候选规范文件及选择原则见 `claude-workflow-governance.md`「Codex Prompt 模板（通用）」中的 context 注入规则。
...
## 开发执行（Phase 6 后）— Legacy / 已废弃

> **⚠️ 已迁移至 `claude-workflow-conductor.md`（Task 级增量执行循环）。**
> 以下旧流程仅作历史参考，新任务必须使用 Conductor。
> 旧流程中的"多轮 Review 协议"已由 Conductor Step 4（Task 级审查）替代。
...
参见 `.claude/workflows/claude-workflow-governance.md` 中的「验证完成门禁（全 workflow 通用）」。

### 6. 自我改进联动

> 参见 `claude-workflow-governance.md` 中的「Self-Improvement 全局规则」

- 任务执行中用户纠正错误 → 立即写入 `.claude/memory/lessons/`
- 任务完成后如有值得记录的模式/陷阱 → 写入 `.claude/memory/lessons/`
