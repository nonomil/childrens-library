...
> `docs/CODEBASE_MAP.md` 是运行期产物，不是模板默认内置文件；若仓库中不存在，应先由 Cartographer 或 `extract` 生成后再进入正式扫描。
...
- 用户显式要求“创建知识图谱”“生成知识图谱”“graphify”“代码图谱”
...
### Step 0：Graphify / Cartographer 双轨预热

> 此步骤为**可选增强**，不阻塞扫描流程；默认按“用户显式要求知识图谱时优先 graphify，本地架构预热优先 Cartographer/`extract`”执行。

1. 满足以下任意一条时，先运行 graphify：
   - 用户显式要求“创建知识图谱 / 生成知识图谱 / graphify / 代码图谱”
   - 项目为 C++/Python 混合且本次改动涉及推理后端替换或跨层重构
     （参见 claude-workflow-cv-codebase.md §4 Step 4）
...
输出自动到 `docs/代码库-知识图谱/{项目名}-LLM图谱/`。

2. graphify 成功后，读取 `docs/代码库-知识图谱/{项目名}-LLM图谱/scan-summary.json` 与 `GRAPH_REPORT.md` 作为预热材料，但**不替代** `docs/CODEBASE_MAP.md`。
3. 再检查 `docs/CODEBASE_MAP.md` 是否存在且 `Generated at` ≤ 7 天。
4. 若已存在且足够新，直接将其加入 `--refs` 参数，进入 Step 1。
5. 若不存在、已过期或用户要求刷新，则先尝试官方 Cartographer 插件生成/更新 `docs/CODEBASE_MAP.md`。
6. 若插件不可用、未启用或执行失败，自动回退到 `largebase-structured-scan extract`，继续产出 `docs/CODEBASE_MAP.md` 与 `extract-data.json`。
7. 仅当用户明确要求跳过预热时，才直接进入 Step 1。
...
如果要声明“插件链路已闭环”，还需要在真实 Claude 环境里按 `.claude/templates/cartographer-e2e-checklist.md` 走一轮 trust / install / generate 验收。
...
操作步骤：
1. 暂停当前任务
2. 修改当前机器使用的 Claude Code API 配置（优先本机覆写，不改模板共享配置）切换为低成本 API profile
   示例配置：
   {
     "api": {
...
1. 定义扫描范围：目录、分支、需求主题。
   ...
   - C++/Python 混合项目（含 pybind11 / Cython 绑定层）必须同时显式列出
     C++ 源码目录（通常为 `src/`）和 Python 源码目录（通常为 `python/`），
     否则 extract 阶段会漏掉其中一种语言的模块节点
...
- 若仓库中没有 `docs/CODEBASE_MAP.md`，应先通过 Cartographer 或 `extract` 生成，再继续本流程。
...
- 调用关系图（仅公共接口）
- 兼容策略（默认值/适配层/双写期）
- 跨语言边界标注（C++/Python 混合项目必填）：
  pybind11 / Cython binding 函数变更必须在 `05-impact-matrix.md`
  中增加「语言边界穿越」列，验证点需同时覆盖 C++ 侧单测和 Python 调用方测试
...
若任何一项失败：
- 禁止进入开发阶段
- 通过 `codex-reply` 要求补齐，不允许 CC 自行脑补

**验证门禁**（进入开发路由前必须自问）：
- ✓ 「Staff Engineer 看到这份扫描包会认为足够完整吗？」
- ✓ 扫描过程中发现的可复用经验/陷阱 → 写入 `.claude/memory/lessons/`
