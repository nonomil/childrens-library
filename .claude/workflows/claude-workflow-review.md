# claude-workflow-review｜Code Review 流程

> 本文档中的所有规则遵循 `claude-workflow-constants.md` 中的全局约束
> 触发条件：用户说"review/审查/检查代码质量/走 review 流程"
> 入口：从 `CLAUDE.md` 场景路由跳转至此
> 若用户明确要求“多专家评审 / 多视角审查 / 并行审查”，或 `expert_review_strategy` 命中自动升级条件，则优先改走 `claude-workflow-multi-review.md`
...
```
Step 1  定位 plan 文档
        - 读取 docs/plan/ 下最新的 YYYY-MM-DD-[feature].md
        - 若不存在，停下报告："未找到 plan.md，请先完成 Phase 1 (Plan 文档生成)"

Step 2  CC 计划审查（Plan Mode，速度优先）
        审查维度：
...
Step 3  Codex 独立审查（工程可行性，独立于 Step 2）
...
Step 4  汇总 + 记录分歧
        - CC 汇总两轮问题，合并去重
        - 分歧点记录到 plan.md 「已知权衡（CC vs Codex 分歧）」表
        - 向用户展示汇总报告，等待用户确认采纳哪些意见

Step 5  更新 plan.md 并确认定稿
        - CC 依据用户确认的意见更新 plan.md
        - plan.md 状态更新为"定稿"
...
> **执行阶段 Review 已迁移至 `claude-workflow-conductor.md` Step 4（Task 级增量审查）**。本文件 Scene 2 适用于非 Conductor 模式。
...
Step 3  Codex Round 2 — 深度审查（质量优先，独立于 Round 1）
        Skill: superpowers:requesting-code-review
        Codex Prompt 模板：
        ──────────────────────────────────────────
        ## Context
...
Step 4  CC 综合仲裁（Round 1 + Round 2 合并）
        - 逐条判断真伪（区分误报和真实问题）
        - 标注最终优先级（P0/P1/P2/P3）
...
Step 5  用户确认 + 分级处置
        ...
        - 确认步骤（Step 5）保证 Claude 不绕过人工直接修复
```
...
Step 2  CC 准备 Review 评论（PENDING 状态，不立即提交）
...
## 与 complex.md / adversarial.md 的关系

| 场景 | 使用文档 |
|------|---------|
| Plan 阶段 Review（Phase 2-3） | **本文件 Scene 1**（更系统化的入口） |
| 代码生成后 Review（Phase 6 执行阶段） | **`conductor.md` Step 4**（Task 级增量审查）；非 Conductor 模式用本文件 Scene 2 |
| PR 合并前 | **本文件 Scene 3** |
| 对抗式协作（Codex-Claude 对战） | **`claude-workflow-adversarial.md`**（替代本文件 Scene 1+2） |

> **分工原则**：本文件是 Review 入口（场景识别 + 分级框架），`conductor.md` Step 4 是执行阶段的 Task 级审查（增量模式），`complex.md` 仅保留计划阶段主线与 Legacy 参考，`adversarial.md` 是对抗博弈模式。

---

## Self-Improvement 联动

- P0/P1 问题根因 → 同步写入 `.claude/memory/lessons/`（防止同类问题复现）
- Review 超过 3 轮仍有新问题 → 写入 `.claude/memory/lessons/`，标注"任务粒度判断失误"
