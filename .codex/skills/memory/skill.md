...
```
.claude/memory/          ← 记忆根目录（Markdown 文件）
├── debug-history.md           ← Bug 根因记录
├── architecture-decisions.md  ← 架构决策
├── codex-lessons.md           ← Codex 调用经验
├── performance-tuning.md      ← 性能优化
├── integration-notes.md       ← 第三方集成经验
└── [自动创建].md              ← 按 title 自动建文件
```
...
首次使用时，确保目录存在：
```bash
mkdir -p .claude/memory/decisions .claude/memory/constraints .claude/memory/patterns .claude/memory/lessons
```
