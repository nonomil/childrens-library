...
<div id="gruffalo-reader" style="max-width: 800px; margin: 0 auto; text-align: center;">
  <div id="page-display" style="position: relative;">
    <img id="page-img" src="../../../images/gruffalo/page_01.png" 
...
function loadPage(n) {
  ...
  const src = `../../../images/gruffalo/page_${String(n).padStart(2, '0')}.png`;
  ...
  for (let i = n - 1; i <= n + 1; i++) {
    if (i >= 1 && i <= totalPages && !imgCache[i]) {
      const pre = new Image();
      pre.src = `../../../images/gruffalo/page_${String(i).padStart(2, '0')}.png`;
      imgCache[i] = pre;
    }
  }
}
