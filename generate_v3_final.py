#!/usr/bin/env python3
"""Simple generator using fixed template - hardcoded data"""
import os, re, json
...
with open(os.path.join(BASE, "generate_courseware.py"), "r") as f:
    code = f.read()
exec(re.search(r'def make_scene_svg[\s\S]*?return svgs\.get\(element, svgs\[\"bus\"\]\)', code).group())
# Read INTERACT_GAMES, QUIZ_DATA, MELODIES
with open(os.path.join(BASE, "generate_courseware_v3.py"), "r") as f:
    v3 = f.read()
exec(re.search(r'INTERACT_GAMES = {[\s\S]*?}^MELODIES', v3, re.MULTILINE).group()[:-8])
exec(re.search(r'QUIZ_DATA = {[\s\S]*?}^MELODIES', v3, re.MULTILINE).group()[:-8])
exec(re.search(r'MELODIES = {[\s\S]*?^}', v3, re.MULTILINE).group())
...
print(f"Loaded {len(SONGS)} songs, make_scene_svg={'ok' if 'make_scene_svg' in dir() else 'MISSING'}")
